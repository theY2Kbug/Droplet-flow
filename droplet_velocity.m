...
for i=1 : 1 : drop_count
    if(mod(i,2)==0)
        plot(ax1,i*lambda, y, 'mo','markerfacecolor','y');
    else
        plot(ax1,i*lambda, y, 'bo','markerfacecolor','g');
    end
end
...
t.TimerFcn = { @fluflow,lambda,beta,ll,ls,ld,Qi,ax1,ax2,ax3};
t.StopFcn = @(~,~)delete(t);
t.Period = 5;
t.TasksToExecute = 30;
...
function fluflow(obj,~,input_speed,b,lenlong,lenshort,ld,Qinput,ax1,ax2,ax3)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    grid(ax2,'on');
    hold(ax2,'on');
    plot(ax2,[getfield(obj.UserData,'global_time') (getfield(obj.UserData,'global_time')+getfield(obj.UserData,'temptime_min'))],[getfield(obj.UserData,'Vshort') getfield(obj.UserData,'Vshort')],'color' ,'r','linewidth',2);
    hold(ax2,'off');
    hold(ax3,'on');
    plot(ax3,[getfield(obj.UserData,'global_time') (getfield(obj.UserData,'global_time')+getfield(obj.UserData,'temptime_min'))],[getfield(obj.UserData,'Vlong') getfield(obj.UserData,'Vlong')],'color' ,'b','linewidth',2);
    hold(ax3,'off');
    ...
    for f = 1 : 1.0 : getfield(obj.UserData,'k')
        ...
        elseif(((prevX+(input_speed*getfield(obj.UserData,'temptime')))>40) && (prevY == 0) && (prevX<40))
            if(getfield(obj.UserData,'Rshort')<getfield(obj.UserData,'Rlong'))
                ...
                %fprintf('Shorter count = %f\n',getfield(obj.UserData,'shortercount'))
                %obj.UserData = setfield(obj.UserData, 'Rshort', lenshort+(getfield(obj.UserData,'shortercount')*ld));
                %fprintf('Rlong=%f > Rshort=%f\n',getfield(obj.UserData,'Rlong'),getfield(obj.UserData,'Rshort'));
        ...
        elseif((prevX>40)&&(prevY==-1.5)&&(prevX<42))
            fprintf('//x-axis temptime = %f , vshort = %f\n',getfield(obj.UserData,'temptime'),getfield(obj.UserData,'Vshort'))
        ...
        elseif((prevX == 42)&&(prevY<0)&&(prevY>-1.5))
            fprintf('//y axis temptime = %f , vshort = %f\n',getfield(obj.UserData,'temptime'),getfield(obj.UserData,'Vshort'))
